...
class TextualInversionDataset(Dataset):
    def __init__(
        ...
        self.image_paths = []
        self.text_paths = {}
        for f in path(self.data_root).files():
            if f.ext != ".txt":
                extlen = len(f.ext)
                textpath = str(f)[0:-extlen] + ".txt"
                if path(textpath).exists():
                    self.text_paths[str(f)] = textpath
                else:
                    print("Warning: no labels found for {}".format(str(f)))
    ...
    def __getitem__(self, i):
        ...
        labelfile = self.text_paths.get(self.image_paths[i % self.num_images])
        if not labelfile:
            text = random.choice(self.templates).format(placeholder_string)
        else:
            with open(labelfile, 'r') as ff:
                text = lazy_intro.format(placeholder_string) + ", " + ff.read()
